...
!function (window) {
    ...
    CitySelect.prototype.loadCity = function () {
        ...
        $.each(cityData, function (k, v) {
            if(typeof v == 'string'){
                arr.push($('<a class="' + (v == _this.defaultSet.city ? 'crt' : '') + '" href="javascript:;"><span>' + v + '</span></a>').data({
                    tag: 1,
                    type: 1
                }));
            } else {
...
!function (window) {
    ...
    InfiniteScroll.prototype.bindScrollEvent = function () {
        ...
        $binder.on('scroll.ydui.infinitescroll', function () {
            //mmp啊，加个硬控防止页面不停的加载（问题解决了，这个硬控先去掉）
            /*if(YDUI.pageScrollEnd){
                _this.$loading.hide();
                _this.loading = false;
                return;
            }*/
    ...
    InfiniteScroll.prototype.loadList = function () {
        ...
        if (typeof options.loadListFn == 'function') {
            ...
            // 监听外部获取数据方法，以便获取数据
            options.loadListFn().done(function (listArr, page, totalPages) {
                ...
                if (len < options.pageSize || page >= totalPages) {
    ...
    function Plugin (option) {
        ...
        if(!oInfiniteScroll){
            oInfiniteScroll = new InfiniteScroll(this, option);
            return oInfiniteScroll;
        }
        oInfiniteScroll.$element = $(this);
        oInfiniteScroll.options = $.extend({}, InfiniteScroll.DEFAULTS, option || {});
